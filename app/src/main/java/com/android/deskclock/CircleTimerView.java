...
public class CircleTimerView extends View {
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (mIntervalStartTime == -1) {
            ...
            if (mMarkerTime != -1 && radius > 0 && mIntervalTime != 0) {
                ...
                canvas.drawArc (mArcRect, 270 + angle, mScreenDensity * (float) (360 / (radius * Math.PI)) , false, mPaint);
            }
    ...
    protected void drawRedDot(Canvas canvas, float degrees, int xCenter, int yCenter, float radius) {
        ...
        canvas.drawCircle(xCenter + (float) (radius * Math.cos(dotRadians)), yCenter + (float) (radius * Math.sin(dotRadians)), mDotRadius, mFill);
    }
